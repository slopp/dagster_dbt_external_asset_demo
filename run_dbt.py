...
def report_dbt_metadata_to_dagster(dbt_model_asset_key, metadata):
    ...
    httpx.post(
        ...
        json={"asset_key": dbt_model_asset_key, "metadata": metadata},
    )
    return
...
def create_duckdb_locations_table():
    ...
    con.execute(
        """ 
            CREATE OR REPLACE TABLE raw_locations AS 
                SELECT * FROM read_csv("./jaffle_shop_duckdb/seeds/raw_customers.csv")
        
        """
    )

    report_dbt_metadata_to_dagster(["main", "raw_locations"], {})
